...
| Kimi Code | yes | yes | yes | no | self-contained session directory + main/subagent wire journals; no target importer yet |
...
ccbridge scan claude codex gemini qwen kiro kimi opencode cursor copilot goose pi kilo --json
```
...
Qwen Code stores the root transcript as an append-only `uuid` / `parentUuid` tree and background subagents under `<projectDir>/subagents/<sessionId>/agent-<id>.jsonl` with `.meta.json` sidecars. Portable mode follows the provider's active leaf and current compression context. Lossless mode preserves inactive root branches, malformed physical records, provider thought parts and subagent transcripts as historical data without replaying tools.
...
Kimi Code stores one replay journal per agent. `agents/main/wire.jsonl` becomes the root conversation and every other `agents/<id>/wire.jsonl` becomes `PortableSession.agents[]`, with parent/fork/labels/swarm metadata taken from `state.json`. Kimi thinking is retained only in lossless mode; `think.encrypted` is preserved as provider-private reasoning state.
...
Kimi Code also currently has no native target importer. `--all` preserves the complete self-contained session directory, including main/subagent wire journals, blob media, plans, goals, tasks, cron state and session logs. The semantic reader follows clear/undo/tool-exchange behavior; when Kimi compaction occurs, ccbridge records `compactionApproximation=true` instead of claiming to reproduce Kimi's internal token-budget user-message selection. See [docs/KIMI.md](docs/KIMI.md).
...
ccbridge transfer qwen opencode <session-id> \
...
See [docs/ADAPTERS.md](docs/ADAPTERS.md), [docs/PORTABLE_SESSION.md](docs/PORTABLE_SESSION.md), [docs/ARCHIVE.md](docs/ARCHIVE.md), [docs/COMPATIBILITY.md](docs/COMPATIBILITY.md), [docs/FIDELITY.md](docs/FIDELITY.md), [docs/KIRO.md](docs/KIRO.md), [docs/KIMI.md](docs/KIMI.md), and [docs/ANTIGRAVITY.md](docs/ANTIGRAVITY.md).
...
Kimi Code:        <KIMI_CODE_HOME|~/.kimi-code>/sessions/<workDirKey>/<sessionId>/
Kimi agents:      <sessionDir>/agents/<agentId>/wire.jsonl
...
Important environment overrides include `CLAUDE_CONFIG_DIR`, `CODEX_HOME`, `GEMINI_CLI_HOME`, `QWEN_HOME`, `QWEN_RUNTIME_DIR`, `CCBRIDGE_QWEN_SESSION_ROOTS`, `KIRO_HOME`, `CCBRIDGE_KIRO_SESSION_ROOTS`, `KIMI_CODE_HOME`, `CCBRIDGE_ANTIGRAVITY_HOME`, `AIDER_CHAT_HISTORY_FILE`, `CCBRIDGE_AIDER_ROOTS`, `CCBRIDGE_ROO_HOME`, `CCBRIDGE_CONTINUE_ROOTS`, `CURSOR_AGENT_HOME`, `CCBRIDGE_VSCODE_CHAT_ROOTS`, `CCBRIDGE_GOOSE_EXPORT_ROOTS`, `PI_CODING_AGENT_DIR`, `PI_CODING_AGENT_SESSION_DIR`, `CCBRIDGE_PI_SESSION_ROOTS`, `CCBRIDGE_KILO_LEGACY_HOME`, `CCBRIDGE_HOME`, and `CCBRIDGE_PASSPHRASE`.
