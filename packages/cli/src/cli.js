...
function printRoutes(matrix) {
  ...
  for (const row of matrix.rows) {
    const detail = row.route === "native"
      ? `native [${row.nativeFormats.map((format) => `${format}:${row.nativePreservation?.[format] ?? "best-effort"}`).join(", ")}]`
      : row.route;
    const targetClass = row.preservation?.targetClass ?? row.route;
    const allClass = row.lossless?.preservation?.overallClass ?? row.lossless?.route ?? "none";
    ...
    console.log(`${row.from} -> ${row.to}: route=${detail}; target=${targetClass}; --all=${allClass}; ${strict}`);
  }
}
...
try {
  ...
  else if (command === "compatibility") { const [adapter, session] = positional(); const result = await bridge.compatibility({ adapterIds: adapter ? [adapter] : [], sessionRef: session ?? null }); print(result); if (result.driftDetected) process.exitCode = 4; }
