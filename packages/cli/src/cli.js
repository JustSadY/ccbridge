...
function positional() { const values = []; const booleanOptions = ["--json", "--dry-run", "--all", "--strict-lossless", "--deep", "--include-raw", "--sessions"]; for (let i = 0; i < args.length; i += 1) { if (args[i].startsWith("--")) { if (!booleanOptions.includes(args[i])) i += 1; continue; } values.push(args[i]); } return values; }
