...
function usage() { console.log(`ccbridge - local coding-agent session bridge\n\nUsage:\n  ccbridge adapters [--json]\n  ccbridge doctor [--json]\n  ccbridge list <adapter> [--json]\n  ccbridge inspect <adapter> <session> [--all]\n  ccbridge fidelity <from> <to> <session> [--all] [--cwd PATH]\n  ccbridge export <adapter> <session> [--output PATH] [--all]\n  ccbridge import <archive.ccbridge> <target-adapter> [--cwd PATH] [--dry-run]\n  ccbridge plan <from> <to> <session> [--cwd PATH] [--all]\n  ccbridge transfer <from> <to> <session> [--cwd PATH] [--all] [--dry-run]\n\nBuilt-ins:\n  claude / claude-code\n  codex\n  gemini / gemini-cli\n  opencode / oc\n  antigravity / agy\n\nExamples:\n  ccbridge fidelity claude opencode <session-id> --all\n  ccbridge transfer claude opencode <session-id> --all\n  ccbridge export antigravity <conversation-id> --output ./agy.ccbridge\n`); }
function transferArgs() { const [from, to, session] = positional(); if (!from || !to || !session) throw new Error("Usage: ccbridge <command> <from> <to> <session> [--cwd PATH]"); return { from, to, session, cwd: valueOf("--cwd"), mode: requestedMode(), bundle: valueOf("--bundle") }; }
try {
