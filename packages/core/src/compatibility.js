...
function knownRecordKind(kind, contract) { if (!contract) return null; const value = String(kind); if ((contract.recordKinds ?? []).includes(value)) return true; if ((contract.recordKindPrefixes ?? []).some((prefix) => value.startsWith(prefix))) return true; if ((contract.recordKindSuffixes ?? []).some((suffix) => value.endsWith(suffix))) return true; return false; }
