const BUILTIN_CONTRACTS = {
  ...
  "qwen-code": { contractVersion: 1, sourceFormats: ["qwen-code/session-jsonl"], recordKinds: ["record:user", "record:assistant", "record:tool_result", "record:system", "malformed-json"], recordKindPrefixes: [], recordKindSuffixes: [":chat_compression", ":slash_command", ":ui_telemetry", ":at_command", ":attribution_snapshot", ":notification", ":cron", ":mid_turn_user_message", ":custom_title", ":parent_session", ":session_source", ":session_model", ":rewind", ":agent_bootstrap", ":agent_launch_prompt", ":agent_retry", ":file_history_snapshot", ":user_text_elements", ":session_artifact_event", ":session_artifact_snapshot", ":branch_checkpoint", ":goal_state", ":goal_runtime", ":realtime_message", ":turn_result"], contentTypes: ["text", "tool-call", "tool-result", "reasoning", "attachment", "qwen-executable-code", "qwen-code-execution-result", "qwen-video-metadata"], preserveUnknownRecords: true, openSourceStorageContract: true, treeStructured: true, compressionAware: true, subagentTranscripts: true, testedVersions: [] },
  "kiro-cli": { contractVersion: 1, sourceFormats: ["kiro-cli/session-jsonl-v1"], recordKinds: ["event:Prompt", "event:AssistantMessage", "event:ToolResults", "event:ToolUse", "event:Clear", "malformed-json"], recordKindPrefixes: [], contentTypes: ["text", "tool-call", "tool-result", "reasoning", "attachment"], preserveUnknownRecords: true, liveJsonlStore: true, clearAware: true, companionMetadata: true, testedVersions: [] },
  "kimi-code": { contractVersion: 1, sourceFormats: ["kimi-code/session-dir-v1"], recordKinds: ["wire:metadata", "wire:context.append_message", "wire:context.append_loop_event", "wire:context.apply_compaction", "wire:context.undo", "wire:context.clear", "wire:context.update_token_count", "wire:full_compaction.begin", "wire:full_compaction.cancel", "wire:goal.create", "wire:goal.update", "wire:plan_mode.enter", "wire:plan_mode.cancel", "wire:plan_mode.exit", "wire:config.update", "wire:profile.bind", "wire:permission.set_mode", "wire:permission.record_approval_result", "wire:tools.update_store", "wire:tools.set_active_tools", "wire:usage.record", "wire:turn.begin", "wire:turn.end", "wire:turn.error", "wire:llm.request", "wire:llm.response", "wire:llm.tools_snapshot", "wire:task.started", "wire:task.terminated", "wire:skill.activate", "wire:interaction.begin", "wire:interaction.end", "wire:token_counting.update", "malformed-json", "torn-tail-json"], recordKindPrefixes: [], contentTypes: ["text", "tool-call", "tool-result", "reasoning", "attachment", "kimi-tool-declaration"], preserveUnknownRecords: true, openSourceStorageContract: true, sessionDirectory: true, subagentTranscripts: true, compactionAware: true, testedVersions: [] },
  ...
  pi: { contractVersion: 1, sourceFormats: ["pi/session-jsonl-v1", "pi/session-jsonl-v2", "pi/session-jsonl-v3"], recordKinds: ["session"], recordKindPrefixes: ["entry:"], contentTypes: ["text", "tool-call", "tool-result", "reasoning", "attachment"], preserveUnknownRecords: true, openSourceStorageContract: true, treeStructured: true, testedVersions: [] },
  "kilo-code": { contractVersion: 1, sourceFormats: ["kilo/session-json", "kilo/legacy-task-files-v1"], recordKinds: ["session-info", "message-info"], recordKindPrefixes: ["part:", "api-message:"], contentTypes: ["text", "tool-call", "tool-result", "reasoning", "attachment"], preserveUnknownRecords: true, officialImportExport: true, hybridBackends: true, testedVersions: [] }
};

function unique(values) { return [...new Set(values.filter((value) => value !== null && value !== undefined).map(String))].sort(); }
function cloneContract(contract) { return contract ? { ...contract, sourceFormats: [...(contract.sourceFormats ?? [])], recordKinds: [...(contract.recordKinds ?? [])], recordKindPrefixes: [...(contract.recordKindPrefixes ?? [])], recordKindSuffixes: [...(contract.recordKindSuffixes ?? [])], contentTypes: [...(contract.contentTypes ?? [])], testedVersions: [...(contract.testedVersions ?? [])] } : null; }
...
function knownRecordKind(kind, contract) {
  if (!contract) return null;
  const value = String(kind);
  const bases = contract.recordKinds ?? [];
  if (bases.includes(value)) return true;
  if ((contract.recordKindPrefixes ?? []).some((prefix) => value.startsWith(prefix))) return true;
  for (const suffix of contract.recordKindSuffixes ?? []) {
...
export async function checkAdapterCompatibility(adapter, options = {}) {
  ...
  report.sessionProbe = {
    ...
    note: (unknownRecordKinds.length || unknownContentTypes.length) && contract?.preserveUnknownRecords ? "Unknown raw records or content blocks are preserved losslessly, but semantic parsing coverage should be reviewed." : contract?.hybridBackends ? "Adapter supports the current official CLI session export/import backend plus the older Roo-compatible VS Code task store. Current data is preferred; ambiguous ids require an explicit backend prefix." : contract?.sessionDirectory ? "Adapter reads the provider's self-contained session directory, main/subagent wire journals and companion state. Portable context follows clear/undo/compaction semantics; raw journals and companions remain available losslessly." : contract?.treeStructured ? "The provider stores an append-only session tree. Portable messages follow the active leaf branch and current compaction context; inactive branches remain preserved as raw lossless events." : contract?.liveJsonlStore ? "Adapter reads Kiro CLI's live versioned JSONL event log plus its metadata/history companions. Portable resume context begins after the latest Clear event; earlier records remain in lossless raw history." : contract?.transcriptMayBeIncomplete ? "The provider transcript is supported as written, but some provider versions omit tool results, usage, timestamps, or reasoning from this artifact." : contract?.presentationOriented ? "Source history is presentation-oriented; semantic role classification is best-effort while the raw section is preserved." : contract?.archivedUpstream ? "Upstream project is archived; compatibility is pinned to the last documented storage contract." : contract?.exportOnly ? "Adapter reads the provider's explicit export artifact; private live-session storage is intentionally not assumed." : contract?.openSourceStorageContract ? "Adapter follows the open-source on-disk chat storage contract and preserves unfamiliar response parts as raw events." : contract?.officialImportExport ? "Adapter uses the provider's official session list/export/import CLI instead of mutating its private database." : null
  };
  return report;
}
