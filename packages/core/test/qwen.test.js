...
async function fixture() {
  ...
  const records = [
    ...
    record({ uuid: "a2", parentUuid: "u2", type: "assistant", timestamp: "2026-09-04T10:00:05.000Z", usageMetadata: { promptTokenCount: 10, candidatesTokenCount: 5 }, message: { role: "model", parts: [{ inlineData: { mimeType: "image/png", data: "aW1hZ2U=" } }, { text: "done" }] } }),
...
test("Qwen lossless mode preserves thinking, inactive branches, malformed input and subagents", async () => {
  const fx = await fixture();
  const adapter = new QwenCodeAdapter({ sessionRoots: [fx.projects], runner: noCli });
