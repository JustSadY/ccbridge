...
async function fixture() {
  ...
  const records = [
    ...
    record({ uuid: "a2", parentUuid: "u2", type: "assistant", timestamp: "2026-09-04T10:00:05.000Z", usageMetadata: { promptTokenCount: 10, candidatesTokenCount: 5 }, message: { role: "model", parts: [{ inlineData: { mimeType: "image/png", data: "aW1hZ2U=" } }, { futureField: { payload: "future-part" } }, { text: "done" }] } }),
...
test("Qwen portable mode follows active branch and current compression context", async () => {
  ...
  assert.ok(session.messages[3].content.some((part) => part.type === "attachment" && part.mimeType === "image/png"));
...
test("Qwen lossless mode preserves thinking, unknown parts, inactive branches, malformed input and subagents", async () => {
  const fx = await fixture();
  const adapter = new QwenCodeAdapter({ sessionRoots: [fx.projects], runner: noCli });
...
test("Qwen compatibility reports unknown GenAI parts as schema drift", async () => {
  const fx = await fixture();
  const adapter = new QwenCodeAdapter({ sessionRoots: [fx.projects], runner: noCli });
