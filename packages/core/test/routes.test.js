...
test("route matrix distinguishes lossless side archives from strict native guarantees", () => {
  const bridge = createDefaultBridge();
  const matrix = bridge.routes();
  ...
  assert.deepEqual(kiloSelf.lossless.strictNativeFormats, ["kilo/session-json"]);
  assert.equal(kiloSelf.lossless.strict, "native-for-listed-formats");
